...
export class ListItemsComponent implements OnInit {
  ...
  addFilter($event: any) {
    ...
    this.filteredClothDataList = this.filteredList.filter((cloth: any) => {
      let colorMatches = colorFilter
        ? colorFilter.length > 0
          ? colorFilter.includes(cloth.color)
          : true
        : true;
      let brandMatches = brandFilter
        ? brandFilter.length > 0
          ? brandFilter.includes(cloth.brand)
          : true
        : true;
      let genderMatches = genderFilter
        ? genderFilter.length > 0
          ? genderFilter.includes(cloth.gender)
          : true
        : true;
      let ratingMatches = ratingFilter
        ? ratingFilter.length > 0
          ? ratingFilter.includes(cloth.rating)
          : true
        : true;
  ...
  addSearchFilter(clothDataListInput: ClothItem[], searchParameter: any) {
    ...
    this.filteredList = clothDataListInput.filter((cloth: any) => {
      let colorMatches = colorFilter
        ? colorFilter.length > 0
          ? colorFilter.includes(cloth.color.toLowerCase())
          : true
        : true;
      let brandMatches = brandFilter
        ? brandFilter.length > 0
          ? brandFilter.includes(cloth.brand.toLowerCase())
          : true
        : true;
      let genderMatches = genderFilter
        ? genderFilter.length > 0
          ? genderFilter.includes(cloth.gender.toLowerCase())
          : true
        : true;
      let categoryMatches = categoryFilter
        ? categoryFilter.length > 0
          ? categoryFilter.includes(cloth.category.toLowerCase())
          : true
        : true;
